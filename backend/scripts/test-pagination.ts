...
async function testPagination() {
  ...
  try {
    ...
    console.log('=== Testando Paginação (6 meses por página) ===\n');
    ...
    for (let page = 1; page <= 2; page++) {
      ...
      const result = await client.query(query, [cityName, perPage, offset]);
      
      console.log(`Meses retornados: ${result.rows.length}\n`);
